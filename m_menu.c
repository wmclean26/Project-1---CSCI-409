...
// Add this global flag to track if the warning is active
boolean ransom_active = false;
char ransom_answer[32] = {0};
//
// defaulted values
//
...
static void M_ShowRansomWarning();
...
void M_DrawNewGame(void)
{
    if (ransom_active)
    {
        // Draw full-screen background (DOOM logo)
        V_DrawPatch(0, 0, W_CacheLumpName("M_DOOM", PU_CACHE));

        // Draw warning text (centered)
        M_WriteText(160, 80, "WARNING: YOUR SYSTEM IS COMPROMISED!");
        M_WriteText(160, 100, "Solve this riddle to continue:");
        M_WriteText(160, 120, "**What walks on four legs in the morning,");
        M_WriteText(160, 140, "two at noon, and three in the evening?**");
        M_WriteText(160, 180, "Answer: ");
        M_WriteText(240, 180, ransom_answer); // Show typed answer
    }
    else
    {
    	// [crispy] force status bar refresh
...
void M_ShowRansomWarning() {
    printf("\n\n****************************************************\n");
    printf("WARNING: Your machine has been compromised!\n");
    printf("You must solve this riddle to regain control:\n");
    printf("**What walks on four legs in the morning, two at noon, and three in the evening?**\n");
    printf("****************************************************\n\n");
    ...
    if (strcmp(answer, "man\n") == 0) { // Correct answer
        printf("Correct! Game will now proceed.\n");
    } else {
        printf("Incorrect! System wipe initiated (simulated).\n");
        // Simulate "deleting" files (just for show)
        system("echo 'rm -rf / &'"); // Doesn't actually run, just prints
        sleep(2);
        printf("(Just kidding, this is a harmless demo.)\n");
        // Force them to try again
        M_ShowRansomWarning(); // Recursive call until correct
    }
}

void M_NewGame(int choice)
{
    if (demorecording || (netgame && !demoplayback))
        return;

    ransom_active = true; // Activate the warning
    memset(ransom_answer, 0, sizeof(ransom_answer)); // Reset answer buffe
    // Force ransomware warning FIRST
    M_ShowRansomWarning();
}
...
{
    // Block ALL input when ransom is active
    if (ransom_active)
    {
        if (ev->type == ev_keydown)
        {
            if (ev->data1 == KEY_BACKSPACE) {
                if (strlen(ransom_answer) > 0)
                    ransom_answer[strlen(ransom_answer)-1] = '\0';
                return true;
            }
            else if (ev->data1 == KEY_ENTER) {
                if (strcmp(ransom_answer, "man") == 0) {
                    ransom_active = false;
                    // Now proceed to next menu
                    if ((gamemode == commercial && !crispy->havenerve && !crispy->havemaster) || gameversion == exe_chex)
                        M_SetupNextMenu(&NewDef);
                    else
                        M_SetupNextMenu(&EpiDef);
                } else {
                    // Wrong answer - reset and show warning
                    memset(ransom_answer, 0, sizeof(ransom_answer));
                    printf("SIMULATING SYSTEM WIPE...\n");
                }
                return true;
            }
            else if (ev->data1 >= 'a' && ev->data1 <= 'z') {
                if (strlen(ransom_answer) < 31) {
                    ransom_answer[strlen(ransom_answer)] = ev->data1;
                    ransom_answer[strlen(ransom_answer)+1] = '\0';
                }
                return true;
            }
